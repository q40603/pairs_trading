...
class trading(object):
    ...
    def __init__( self , table , formate_time , min_data , tick_data , open_time , stop_loss_time , day1 , maxi , tax_cost , capital ):
    ...
    def backtest_table(self):
        ...
        for j in range(n):
            ...
            y = self.table.iloc[j,:]
            pool.apply_async( pairs , ( j , self.formate_time , y , self.min_data , self.tick_data , self.open_time , self.stop_loss_time , self.day1 , self.maxi , self.tax_cost , self.capital,) , callback=self.append_backtest_Result )
            
            #result.append(pairs(j , self.formate_time , y , self.min_data , self.tick_data , self.open_time , self.stop_loss_time , self.day1 , self.maxi , self.tax_cost , self.capital))
        ...
        if self.tax_cost == 0:
            ...
            self.profit.columns = ["profit without cost"]
        ...
        #self.pos = pd.DataFrame(self.pos)             ; self.pos.columns = ["position"]
        
        back_test = pd.concat([self.stock1_name,self.stock2_name,self.profit,self.open_num,self.rt] , axis = 1)
