...
#    day = ["09"]
    ...
    save_path = r'C:\Users\MAI\Desktop\trading_result\after20200204\trading_period_matrix_ver1.0\table\20200619testcorrect'
    ...
    for z in range(len(year)):
        ...
        for m in range(len(month)):
            ...
            for i in range(len(day)):
                ...
                date = ''.join( [ year[z] , month[m] , day[i] ] ) ; Date = year[z] + "/" + str(int(month[m])) + "/" + str(int(day[i]))
                
                # 讀取資料 --------------------------------------------------------------------------------------------------------------------------------
                try:
                
                    # 讀取台股資料(day1)
                    day1_0050 = pd.read_csv( ''.join([ "C:/Users/MAI/Desktop/", year[z] ,"/averageprice/" , date , "_averagePrice_min.csv"]) , encoding="utf-8")
                ...
                for j in range(1):    # 一天建模??次
                    ...
                    table.to_csv( ''.join([ save_path ,'/', date ,"_table.csv" ]) , index = False )                  #寫入方式選擇wb，否則有空行               
