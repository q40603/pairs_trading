...
#from formation_period_with_timetrend import find_pairs
#from formation_period_with_timetrend import find_pairs
...
#        for i in range(n):
#            self.append_pairs_Result(find_pairs(i,n,self.data))
		#self.data = data
        ...
        self.pval = pd.DataFrame(self.pval); self.pval.columns = ["pval"]
    ...
    def form_table(self):
        ...
        con = pd.concat([self.name,self.select_model,self.weight,self.pval],axis=1)  
...
#            self.s_n_r.append( snr(y,100) )
        self.s_n_r = np.zeros(len(self.spread.T))
