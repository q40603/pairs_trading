# -*- coding: utf-8 -*-
"""
Created on Sun Feb 23 17:15:04 2020
...
def CNN_test(st1,st2,sp,v1,v2,tick,DetPos,table,NowOpen):
    ...
    for m in range(times):
        ...
        for i in range(lenth):
            ...
            SprInput[:,5] = st1[pair,:100]
            SprInput[:,6] = st2[pair,:100]
            SprInput[:,7] = sp[pair,(s+index):(s+100+index)]
            SprInput[:,8] = v1[pair,:100]
            SprInput[:,9] = v2[pair,:100]

            CharInput[4] = index/100
    ...
    for month in months:
        ...
        for day in days:
            try:
                ...
                test_volume = test_volume.iloc[form_del_min:,:]
            ...
            for i in range(len(arg_mix)):
                ...
                if condiction == 1:
                    ...
                    if tick:
                        CNNDetPos_test.append( (arg_mix[i] + whereDet)//12 )
                    else:
...
if CNN_open:
    ...
    Open_table_index = np.arange(len(prediction))[prediction]
    table = table.iloc[Open_table_index,:]
    arg_test = arg_test[Open_table_index]
    arg_mix = arg_mix[Open_table_index]
    ClPos = ClPos[Open_table_index]
