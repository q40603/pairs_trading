...
tick_trigger_dataPath = r'D:\tick(secs)\{}'.format(y)
origin_trigger_dataPath = r'C:\Users\MAI\Desktop\{}\minprice'.format(y)
                ...
                for i in range(len(arg_mix)):
                    if arg_mix[i] != 0:
                        ...
                        p,A,ut,_ = fore_chow(test_stock1[i,:(150*(1+half)+1)],
                                             ...
                                             150*(1+half))
                        for j in range(arg_mix[i],TpLen-5):
                            p,A,ut,StrBr = fore_chow(test_stock1[i,:(150*(1+half)+j+2)],
                                                     test_stock2[i,:(150*(1+half)+j+2)],
                                                     table.model_type.iloc[i],
                                                     150*(1+half),True,
                                                     p,A,ut)
                            if forecast:
                                if StrBr == 1:
                                    ForecastPos[i] = j+1
                                    ForePrice[i,0] = trigger_stock1[i,(j+2+CloseSlipTick)]
                                    ForePrice[i,1] = trigger_stock2[i,(j+2+CloseSlipTick)]
                                    ForePrice[i,2] = trigger_spread[i,(j+2+CloseSlipTick)]
                                    break
                            else:
                                if StrBr == 1:
                                    count = count + 1
