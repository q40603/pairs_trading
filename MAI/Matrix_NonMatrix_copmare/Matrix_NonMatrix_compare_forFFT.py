# -*- coding: utf-8 -*-
"""
Created on Thu Feb 27 01:03:41 2020
...
def CNN_test(st1,st2,sp,v1,v2,tick,DetPos,table,NowOpen):
    ...
    for m in range(times):
        SprInput = np.zeros([63])
        FSprInput = np.zeros([63])
        if not NowOpen:
            lenth = len(DetPos[m])
        else:
            lenth = len(DetPos)
        for i in range(lenth):
            ...
            _,SprInput[:51] = get_psd_values(standard(sp[pair,(s+index):(s+100+index)]),1/25,100,25)
            _,FSprInput[:] = get_psd_values(standard(sp[pair,25:150]),1/25,100,25)            
            count += 1
            AllSprInput.append(SprInput.copy())
            AllFSprInput.append(FSprInput.copy())
        pair_pos[m] = count
    AllSprInput = np.array(AllSprInput)
    AllCharInput = np.array(AllCharInput)
    
    #Normalize
    All = np.zeros([len(AllSprInput),63,2])
    All[:,:,0] = AllSprInput
    All[:,:,1] = AllFSprInput
    m = np.max(All,axis=1)
    m = np.max(m,axis=1)
    
    All = All/m[:,np.newaxis,np.newaxis]

    #CNN_predict
    pre = model_CNN.predict([All])
...
half = False
EarlyStop = 60*(1+half)              # After how many minutes, we stop opening spread 
...
CNN_open = False             # Whether to use CNN-check before open
CNN_det = False              # Whether to use CNN-check during trading period
zcr = False
...
maxi = 10                    # restrict the number of ticket we could buy for a stock
